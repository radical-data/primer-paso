...
const parseApplicantConfirmation = (
	formData: FormData,
	memberId: string,
	now = new Date()
): CertificateApplicantConfirmation => {
	const method = getString(formData, 'confirmationMethod')
	if (!isCertificateApplicantConfirmationMethod(method)) {
		throw new Error('Invalid applicant confirmation method')
	}
	return {
		informationAccurate: formData.get('informationAccurate') === 'on',
		understandsNotIssuedByPrimerPaso: formData.get('understandsNotIssuedByPrimerPaso') === 'on',
		understandsOrganisationWillReview: formData.get('understandsOrganisationWillReview') === 'on',
		consentsToDataUseForCertificate: formData.get('consentsToDataUseForCertificate') === 'on',
		consentsToStoreForAuditAndIssue: formData.get('consentsToStoreForAuditAndIssue') === 'on',
		method,
		confirmedAt: now.toISOString(),
		confirmedByMemberId: method === 'public_handoff' ? undefined : memberId
	}
}
...
const certificateDraftToFormValue = (draft: CertificateDraft) => ({
	givenNames: draft.userData.identity.givenNames,
	familyNames: draft.userData.identity.familyNames,
	documentType: draft.userData.identity.documentType,
	documentNumber: draft.userData.identity.documentNumber,
	dateOfBirth: draft.userData.identity.dateOfBirth ?? '',
	nationality: draft.userData.identity.nationality ?? '',
	email: draft.userData.contact.email,
	phone: draft.userData.contact.phone ?? '',
	addressLine1: draft.userData.location.addressLine1,
	addressLine2: draft.userData.location.addressLine2 ?? '',
	municipality: draft.userData.location.municipality,
	province: draft.userData.location.province,
	postalCode: draft.userData.location.postalCode ?? '',
	vulnerabilityReasons: draft.userData.vulnerability.reasons
})
...
export const load: PageServerLoad = async ({ locals, params }) => {
	...
	if (!review || review.organisationId !== session.organisationId) {
		throw error(404, 'Revisión no encontrada.')
	}
	...
	return {
		review: {
			...
			applicantConfirmation: review.applicantConfirmation,
			verification: review.verification,
			draftSnapshot: parseCertificateDraft(review.draftSnapshot)
		},
		formValue: certificateDraftToFormValue(draft),
		...
		correctionTypeOptions: [
			...
			{ value: 'standardised_format', label: 'Formato normalizado' },
		...
		canIssueCertificate:
			...
			!issuedCertificate,
		certificateHref: issuedCertificate ? `/certificates/review/${review.id}/certificate.pdf` : null
	}
}

export const actions: Actions = {
	applicantConfirmation: async ({ locals, params, request }) => {
		...
		const now = new Date()
		const applicantConfirmation = parseApplicantConfirmation(formData, session.memberId, now)
		if (!applicantConfirmationComplete(applicantConfirmation)) {
			return fail(400, {
				applicantConfirmationError:
					'All applicant confirmation statements must be accepted before the certificate can be issued.'
			})
		}
		const review = await repository.updateCertificateHandoffReviewApplicantConfirmation({
			reviewId: params.id,
		...
		await writeAuditEvent({
			eventType: 'certificate_review.applicant_confirmation_updated',
			eventData: {
				method: applicantConfirmation.method,
				origin: review.origin
			},
	...
	updateReviewedData: async ({ request, params, locals }) => {
		...
		if (!updated) {
			throw error(409, 'No se pudo actualizar la revisión.')
		}
	...
	ready: async ({ locals, params, request }) => {
		...
		if (
			...
			!applicantConfirmationComplete(review.applicantConfirmation)
		) {
			return fail(400, {
				error:
					'Completa la confirmación de la persona solicitante y todas las confirmaciones de verificación antes de marcar esta revisión como lista para emitir.',
				verification
			})
		}
	...
	issue: async ({ locals, params, request }) => {
		...
		if (!applicantConfirmationComplete(review.applicantConfirmation)) {
			return fail(400, {
				issueError: 'Applicant confirmation is required before issuing the certificate.'
			})
		}
		...
		if (!organisation || !signer || signer.organisationId !== organisation.id) {
			...
			throw error(409, 'No se pudieron resolver los datos de la organización o del firmante.')
		}
